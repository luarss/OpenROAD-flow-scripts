...
if __name__ == "__main__":
    ...
    for design in ["gcd", "ibex", "aes"]:
        for platform in ["asap7", "sky130hd", "ihp-sg13g2"]:
            LOG_DIR = os.path.join(
                CUR_DIR, f"../../../../../flow/logs/{platform}/{design}/"
            )
            ...
            for i, path in enumerate(paths):
                ...
                if len(os.listdir(path)) < 200:
                    print(
                        f"Should have ~100 trials, only have {len(os.listdir(path))/2}"
                    )
                    continue
                ...
                print(
                    f"Total runtime: {total_runtime:.3f} s, Peak runtime: {peak_runtime:.3f} s"
                )
